...
$factura_sets = json_decode(file_get_contents("datasets/data_sets.json"),true);
// lista de estados

function provincia($estado){
$_estado = (double)$estado;
$key = $_estado.'';
return $GLOBALS['factura_sets']['provincias'][$key];
}
function canton($estado,$ciudad) {
$_estado = (double)$estado;
$_ciudad = (double)$ciudad;
$key = $_estado . '.' . $_ciudad;
return $GLOBALS['factura_sets']['cantones'][$key];
}
...
return $GLOBALS['factura_sets']['distritos'][$key];
}
...
function identificacion_nombre($type_str){
        $type_number = (double)$type_str;
    return $GLOBALS['factura_sets']['identificacion_type'][$type_number];
}

//xml en formato array
...
$dir = parse_ini_file('config.ini');    

//formatea valores resumen de la factura
...
$emisorNombreComercial = get_elem($xml_array,get_elem($xml_array,'','Emisor','Nombre'),'Emisor','NombreComercial');//isset($xml_array['Emisor']['NombreComercial'])==true?$xml_array['Emisor']['NombreComercial']:$xml_array['Emisor']['Nombre'];
...
$emisorUbicacionOtrasSenas = get_elem($xml_array,'.','Emisor','Ubicacion','OtrasSenas');//isset($xml_array['Emisor']['Ubicacion']['OtrasSenas'])==true?$xml_array['Emisor']['Ubicacion']['OtrasSenas']:".";
...
$receptorUbicacionOtrasSenas = get_elem($xml_array,'.','Receptor','Ubicacion','OtrasSenas');//isset($xml_array['Receptor']['Ubicacion']['OtrasSenas'])==true?$xml_array['Receptor']['Ubicacion']['OtrasSenas']:".";
...
// use the template
ob_start();
require($dir['template_dir']."header.php");
echo '<thead>
<tr>';
$descuentos = 0;
$codigos = 0;
...
foreach ($lines as $line){
    ...
    $codigoCodigo = get_elem($line,'','Codigo','Codigo');//isset($line['Codigo'])==true && isset($line['Codigo']['Codigo'])==true?$line['Codigo']['Codigo']:'';
    $cantidad = cantidad_format(get_elem($line,'1','Cantidad'));//number_format(get_elem($line,'1','Cantidad'),2,".",",");
    ...
    $naturalezaDescuento = get_elem($line,'','NaturalezaDescuento');//isset($line['NaturalezaDescuento']) == true?$line['NaturalezaDescuento']:'';
...
require($dir['template_dir']."footer.php");
...
function emailFactura($clave){
$dir = parse_ini_file('config.ini');    
    
$xml_factura = $dir['facturas_dir'].'factura'.$clave.".xml";
$xml_response = $dir['facturas_dir'].'xml-response'.$clave.".xml";
$xml_array = xml_to_array($xml_factura);
$pdf_string = rendered_factura($xml_array);

$email = parse_ini_file($dir['email_dir']);
...
$bodyHTML = $body;
$xml_array['Emisor']['NombreComercial'] = $emisorNombreComercial; //isset($xml_array['Emisor']['NombreComercial'])==true?$xml_array['Emisor']['NombreComercial']:$xml_array['Emisor']['Nombre'];
